...
from .session import KedroSessionWithPipelineName
...
@click.option("--namespace", type=str, default=None, help="Namespace of the pipeline")
@click.pass_obj
def run_command(
    ctx,
    pipeline: str,
    ...
    # Convert load_version tuple to dict format expected by Kedro
    load_versions = None
    if load_version:
        load_versions = {}
    ...
    # Get conf_source from context if available, otherwise let KedroSession use defaults
    conf_source = getattr(ctx, "conf_source", None)
    env_value = env or getattr(ctx, "env", None)

    with KedroSessionWithPipelineName.create(
        env=env_value,
        ...
        session.run(
            ...
            tags=None,  # Add tags support if needed
            runner=FusedRunner(),
